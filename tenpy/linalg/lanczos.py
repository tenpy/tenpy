...
class LanczosGroundState:
    ...
    def run(self):
        ...
        if N > 1:
            ...
            logger.debug(msg.format(N, self._T[0, 0], self._T[0, 1]))
        if self.E_shift is not None:
            E0 -= self.E_shift
        if N == 1:
    ...
    def _calc_result_full(self, N):
        ...
        if abs(1. - psif_norm) > 1.e-5:
            ...
            logger.warning("poorly conditioned Lanczos! |psi_0| = {0:f}".format(psif_norm))
...
class LanczosEvolution(LanczosGroundState):
    ...
    def run(self, delta):
        ...
        if N > 1:
            ...
            logger.debug(msg.format(N, abs(self._result_krylov[-1])))
        else:
            msg = "Lanczos N={0:d}, first alpha={1:.3e}, beta={2:.3e}"
            logger.debug(msg.format(N, self._T[0, 0], self._T[0, 1]))
        if N == 1:
