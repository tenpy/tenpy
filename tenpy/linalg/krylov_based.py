...
class KrylovBased:
    ...
    def __init__(self, H, psi0, options):
        ...
        self.P_tol = options.get('P_tol', 1.0e-14, 'real')
        self.min_gap = options.get('min_gap', 1.0e-12, 'real')
...
class GMRES:
    def __init__(self, A, x, b, options):
        ...
        self.res = self.options.get('res', 1.0e-8, 'real')
        self.A = A
        self.b = b.copy()
        ...
        self.e1 = npc.Array.from_ndarray_trivial(np.zeros(self.N_max + 1) * 1.0j)
        self.e1[0] = 1
    ...
    def run(self):
        ...
        for _ in range(self.restart):
            ...
            for k in range(0, self.N_max):
                ...
                error = (
                    np.abs(self.e1[k + 1]) / self.b_norm
        ...
        return (
            self.x,
            npc.norm(self.A.matvec(self.x) - self.b) / self.b_norm,
            self.total_error,
            self.total_iters,
        )
    ...
    def arnoldi(self, k):
        ...
        if self.H[k + 1, k] > 0:  # avoid warning if norm(q)==0, error=0 in that case
            q.iscale_prefactor(1.0 / self.H[k + 1, k])
        self.qs.append(q)
...
class Arnoldi(KrylovBased):
    ...
    def _build_krylov(self):
        ...
        for k in range(self.N_max):
            self.iscale_prefactor(w, 1.0 / norm)
            self._to_cache(w)
            w = self.H.matvec(w)
    ...
    def _calc_result_full(self, N):
        ...
        for i in range(min(N, self.num_ev)):
            ...
            self.iscale_prefactor(psi, 1.0 / psi_norm)
            psis.append(psi)
        return psis
    ...
    def _converged(self, k):
        ...
        gap = max(
            min([np.min(np.abs(E[i + 1 :] - E[i])) for i in range(self.num_ev)]), self.min_gap
        )
        P_err = (RitzRes / gap) ** 2
    ...
    **Assumes** that `H` is hermitian.
    ...
    def run(self):
        ...
        if N > 1:
            ...
            logger.debug(
                'Lanczos N=%d, first alpha=%.3e, beta=%.3e',
                N,
                self._h_krylov[0, 0],
                self._h_krylov[0, 1],
            )
    ...
    def _build_krylov(self):
        ...
        for k in range(self.N_max):
            self.iscale_prefactor(w, 1.0 / beta)
            self._to_cache(w)
            w = self.H.matvec(w)
...
def lanczos_arpack(H, psi, options={}):
    ...
    tol = options.get('P_tol', 1.0e-14, 'real')
...
def gram_schmidt(vecs, rcond=1.0e-14):
