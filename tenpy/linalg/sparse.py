...
class SumNpcLinearOperator(NpcLinearOperatorWrapper):
    ...
    def matvec(self, vec):
        if isinstance(vec, npc.Array):
            ...
            return [
                a + b
                for a, b in zip(self.orig_operator.matvec(vec), self.other_operator.matvec(vec))
            ]
...
class BoostNpcLinearOperator(NpcLinearOperatorWrapper):
    ...
    def matvec(self, vec):
        ...
        for b, bv in zip(self.boosts, self.boost_vecs):
            krylov_based.iadd_prefactor_other(
                temp, b * npc.inner(bv, vec, axes='range', do_conj=True), bv
            )
        return temp
        # return self.orig_operator.matvec(vec) + self.shift * vec
    ...
    def adjoint(self):
        return BoostNpcLinearOperator(
            self.orig_operator.adjoint(), np.conj(self.boosts), self.boost_vecs
        )
...
class OrthogonalNpcLinearOperator(NpcLinearOperatorWrapper):
    ...
    def __init__(self, orig_operator, ortho_vecs):
        if len(ortho_vecs) == 0:
            warnings.warn(
                'Empty `ortho_vecs`: no need to patch `OrthogonalNpcLinearOperator`', stacklevel=2
            )
        super().__init__(orig_operator)
    ...
    def matvec(self, vec):
        ...
        for o in self.ortho_vecs:  # Project out
            ...
            krylov_based.iadd_prefactor_other(
                vec, -npc.inner(o, vec, axes='range', do_conj=True), o
            )
...
class FlatLinearOperator(ScipyLinearOperator):
    ...
    def __init__(self, npc_matvec, leg, dtype, charge_sector=0, vec_label=None, compact_flat=None):
        ...
        if compact_flat is None:
            ...
            if not leg.is_blocked():
                raise ValueError(
                    'FlatLinearOperator with `compact_flat` works only ' 'for blocked `leg`.'
                )
    ...
    @classmethod
    def from_guess_with_pipe(
        cls, npc_matvec, v0_guess, labels_split=None, dtype=None, compact_flat=True
    ):
    ...
    def flat_to_npc(self, vec):
        ...
        if self._charge_sector is not None:
            ...
            leg = self.leg
            ch_leg = npc.LegCharge.from_qflat(
                leg.chinfo, self.possible_charge_sectors, qconj=-leg.qconj
            )
            res = npc.zeros([self.leg, ch_leg], vec.dtype, labels=[self.vec_label, 'charge'])
            res._qdata = np.repeat(np.arange(leg.block_number, dtype=np.intp), 2).reshape(
                leg.block_number, 2
            )
    ...
    def eigenvectors(
        ...
        **kwargs :
        ...
        from_ndarray_args = dict(
            legcharges=[self.leg], cutoff=cutoff, labels=[self.vec_label], raise_wrong_sector=True
        )
        ...
        if self._charge_sector is not None:
            ...
            for degenerate in group_by_degeneracy(eta, cutoff=cutoff):
                ...
                for _ in range(len(degenerate)):
                    ...
                    max_qi, max_j = np.unravel_index(
                        np.argmax(sector_norms, axis=None), sector_norms.shape
                    )
                    j = degenerate[max_j]
