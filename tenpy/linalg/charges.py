...
class LegCharge:
    ...
    def get_qindex(self, flat_index):
        ...
        if flat_index < 0:
            ...
            raise IndexError(
                f'flat index {flat_index:d} too large for leg with ind_len {self.ind_len:d}'
            )
    ...
    def __repr__(self):
        """Full string representation."""
        return (
            f'LegCharge({self.chinfo!r}, qconj={self.qconj:+d},\n{self.slices!r}, {self.charges!r})'
        )
...
class LegPipe(LegCharge):
    ...
    def apply_charge_mapping(self, map_func, func_args=(), func_kwargs={}):
        ...
        res = self.copy()
        res.legs = [
            l.apply_charge_mapping(map_func, func_args=func_args, func_kwargs=func_kwargs)
            for l in self.legs
        ]
        res.charges = map_func(self.charges, *func_args, **func_kwargs)
        res.sorted = res.bunched = False
    ...
    def __str__(self):
        ...
        res_lines = [
            ...
            f'    qconj ({", ".join("%+d" % l.qconj for l in self.legs)})->{self.qconj:+1};',
