...
class Array:
    ...
    def test_sanity(self):
        ...
        for l in self.legs:
            if l.chinfo != self.chinfo:
                raise ValueError(
                    f'leg has different ChargeInfo:\n{l.chinfo!s}\n vs {self.chinfo!s}'
                )
        if self.shape != tuple([lc.ind_len for lc in self.legs]):
            raise ValueError(
                f'shape mismatch with LegCharges\n self.shape={self.shape!s} != {tuple([lc.ind_len for lc in self.legs])!s}'
            )
        for l in self.legs:
            l.test_sanity()
        if any([self.dtype != d.dtype for d in self._data]):
            raise ValueError(
                f'wrong dtype: {self.dtype!s} vs\n {[self.dtype != d.dtype for d in self._data]!s}'
            )
    ...
    def from_ndarray(
        ...
        if res.shape != data_flat.shape:
            raise ValueError(
                f'Incompatible shapes: legcharges {res.shape!s} vs flat {data_flat.shape!s} '
            )
        if qtotal is None:
    ...
    @classmethod
    def from_func_square(
        cls, func, leg, dtype=None, func_args=(), func_kwargs={}, shape_kw=None, labels=None
    ):
    ...
    def shift_charges(self, dx, inplace: bool = False):
        ...
        if self.chinfo.trivial_shift or np.all(np.equal(dx, 0)):
            return self
        return self.apply_charge_mapping(
            self.chinfo.shift_charges, func_kwargs=dict(dx=dx), inplace=inplace
        )
    ...
    def shift_charges_horizontal(self, dx_0: int, inplace: bool = False):
        ...
        if self.chinfo.trivial_shift or dx_0 == 0:
            return self
        return self.apply_charge_mapping(
            self.chinfo.shift_charges_horizontal, func_kwargs=dict(dx_0=dx_0), inplace=inplace
        )
    ...
    def combine_legs(self, combine_legs, new_axes=None, pipes=None, qconj=None):
        ...
        if self.stored_blocks == 1:
            ...
            _combine_legs_worker(
                self, res, combine_legs, non_combined_legs, new_axes, non_new_axes, pipes
            )
        return res
            ...
            for old_data_index in np.nonzero(qdata_axis == old_qind)[0]:
                ...
                for i_old in old_range:
                    ...
                    if new_data_ind == len(data):
                        # insert new block
                        data.append(
                            np.zeros(res._get_block_shape(new_qindices), dtype=old_block.dtype)
                        )
    ...
    def iscale_axis(self, s, axis=-1):
        ...
        if s.shape != (self.shape[axis],):
            raise ValueError(
                's has wrong shape: ' + str(s.shape) + ' instead of ' + str(self.shape[axis])
            )
            ...
            if isinstance(i, slice):
                if i != slice(None):
                    ...
                    if i.step is not None and i.step < 0:
                        permutations.append(
                            (a, np.arange(np.count_nonzero(m), dtype=np.intp)[::-1])
                        )
            else:
                try:
                    iter(i)
    ...
    def _advanced_getitem_map_qind(
        self, inds, slice_axes, slice_inds, project_axes, p_map_qinds, p_masks, res_axes
    ):
    ...
    def _transpose_same_labels(self, other_labels):
        ...
        if None in self_labels or None in other_labels:
            if set(self_labels) == set(other_labels) != set([None]):
                warnings.warn(
                    'Not all legs labeled, so no transpose for Array addition. '
...
def detect_grid_outer_legcharge(grid, grid_legs, qtotal=None, qconj=1, bunch=False):
    ...
    for idx, entry in entries:
        grid_charges = [
            l.get_charge(l.get_qindex(i)[0])
            for a, (i, l) in enumerate(zip(idx, grid_legs))
            if a != axis
        ]
    ...
    grid_legs[axis] = LegCharge.from_qflat(
        chinfo, chinfo.make_valid(qconj * np.array(qflat)), qconj
    )
    return grid_legs
...
def detect_qtotal(flat_array, legcharges, cutoff=None):
    ...
    if val_max < cutoff:
        warnings.warn(
            "can't detect total charge: no entry larger than cutoff. Return 0 charge.", stacklevel=2
        )
...
def svd(
    ...
    U, S, VH = _svd_worker(
        a, full_matrices, compute_uv, overwrite_a, cutoff, qtotal_LR, inner_qconj
    )
    if not compute_uv:
        return S
...
def polar(a, cutoff=1.0e-16, left=False, inner_labels=[None, None]):
    ...
    if not left:
        ...
        # a = up
        p = tensordot(VH.conj().itranspose().iscale_axis(s), VH, axes=([1, 0])).iset_leg_labels(
            labels
        )
        ...
        p = tensordot(W.iscale_axis(s), W.conj().itranspose(), axes=([1, 0])).iset_leg_labels(
            labels
        )
        # p = (w * s).dot(w.T.conj())
    return u, p, s
...
def orthogonal_columns(a, new_label=None):
    ...
    if M == N:
        ...
        right_leg = LegCharge(
            a.chinfo, [0], np.zeros([0, a.chinfo.qnumber], dtype=QTYPE), a.legs[1].qconj
        )
    ...
    right_charges = a.chinfo.make_valid(
        right_qconj * (a.qtotal - left_leg.get_charge(right_kept_blocks))
    )
...
def _split_legs_worker(self, split_axes, cutoff):
    ...
    for j in range(N_split):
        ...
        q_map_slices_shape[:, j] = q_map_slices[
            qinds + 1
        ]  # - q_map_slices[qinds] # one line below # TODO: in pipe
...
def _tensordot_pre_reshape(data, cut, dtype, same_shape_before_cut=True):
    ...
    if cut == 0 or cut == data[0][0].ndim:
        ...
        return [
            [np.reshape(T, (-1,)).astype(dtype, order='F', copy=False) for T in blocks]
            for blocks in data
        ]
    res = []
    for blocks in data:
...
def _tensordot_worker(a, b, axes):
    ...
    if a.stored_blocks == 0 or b.stored_blocks == 0:  # special case: `a` or `b` is 0
        return zeros(
            a.legs[:-axes] + b.legs[axes:], np.promote_types(a.dtype, b.dtype), a.qtotal + b.qtotal
        )
    cut_a = a.rank - axes
    cut_b = axes
    ...
    for col_b, charge_match in enumerate(b_charges_match):
        ...
        for row_a in rows_a:
            block_contr = fast_dot_sum(
                a_data[row_a], b_data[col_b], a_qdata_contr[row_a], b_qdata_contr[col_b]
            )
    ...
    for i in range(len(a._data)):
        ...
        if compute_uv:
            ...
            if anynan(U_b) or anynan(VH_b) or anynan(S_b):
                ...
                if anynan(U_b) or anynan(VH_b) or anynan(S_b):
                    raise ValueError(
                        f'NaN in U_b {np.sum(np.isnan(U_b)):d} and/or VH_b: {np.sum(np.isnan(VH_b)):d}'
                    )
        else:
