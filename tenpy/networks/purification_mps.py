...
class PurificationMPS(MPS):
    ...
    def from_density_matrix(cls, sites, rho, form=None, cutoff=1e-16, normalize=True):
        ...
        """
        L = len(sites)
        rho = rho.combine_legs(
            [[f'p{i}' for i in range(L)], [f'p{i}*' for i in range(L)]]
        )  # [P, P*]
        ...
        return cls.from_full(
            sites, psi, form=form, cutoff=cutoff, normalize=normalize, bc='finite', outer_S=None
        )

    @classmethod
    def from_density_matrix(
        cls, sites, rho, form=None, cutoff=1e-16, normalize=True, unit_cell_width=None
    ):
        r"""Construct a purification from a single tensor `rho` of the density matrix.
    ...
    def from_infiniteT_canonical(
        ...
        assert all(
            s.leg.chinfo == chinfo for s in sites
        ), 'Charge Info for all sites must be identical'
    ...
    def sample_measurements(
        ...
        for i in range(first_site, last_site + 1):
            ...
            if not sample_q:
                ...
                theta = theta.take_slice(
                    sigma, 'p'
                )  # project to sigma in theta; now has legs vL (trivial), q, vR
                probability = rho_diag[
                    sigma
                ]  # this is probability of seeing sigma conditioned on previous results.
                ...
                sigma_1 = rng.choice(
                    site.dim, p=rho_diag
                )  # randomly select index from probabilities
                probability = rho_diag[
                    sigma_1
                ...
                theta = theta.take_slice(
                    [sigma_1], ['p']
                )  # project to sigma in theta; now has legs vL (trivial), vR

                # Sample q
                rho = npc.tensordot(
                    theta.conj(), theta, [['vL*', 'vR*'], ['vL', 'vR']]
                )  # physical RDM on site i
                # probabilities p(sigma) = <sigma|rho|sigma>
                rho_diag = np.abs(np.diag(rho.to_ndarray()))  # abs: real dtype & roundoff err
            ...
            if i != last_site:
                ...
                assert theta.shape == (
                    1,
                    1,
                )  # This contains the phase; but we don't want this since
    ...
    if hasattr(model, 'H_MPO'):
        ...
        for W in H_MPO._W:
            ...
            for i in range(3):
                ...
                W.legs[i] = npc.LegCharge.from_qind(
                    chinfo_cac, leg.slices, chinfo_cac.make_valid(Q), leg.qconj
                )
