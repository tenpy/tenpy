...
class BaseMPSExpectationValue(MPSGeometry, metaclass=ABCMeta):
    ...
    def term_correlation_function_right(
        self, term_L, term_R, i_L=0, j_R=None, autoJW=True, opstr=None
    ):
    ...
    def term_correlation_function_left(
        self, term_L, term_R, i_L=None, j_R=0, autoJW=True, opstr=None
    ):
    ...
    def term_list_correlation_function_right(
        self, term_list_L, term_list_R, i_L=0, j_R=None, autoJW=True, opstr=None
    ):
        ...
        **Assumes** that overall terms with an odd number of operators requiring a Jordan-Wigner
        ...
        for j in j_R:
            ...
            for k in range(i, j):
                ...
                i = k + 1
            res = 0.0
            for ops_R, need_JW, term_R, strength in zip(
                all_ops_R, need_JW_R, term_list_R.terms, term_list_R.strength
            ):
                chinfo = self.sites[0].leg.chinfo
                if not chinfo.trivial_shift:
                    # recalculate operators
                    ops_R, j_min, need_JW = self._term_to_ops_list(
                        term_R, autoJW, j
                    )  # <- note the j
                    if j_min > j + min_R:
...
class MPS(BaseMPSExpectationValue):
    ...
    def test_sanity(self):
        ...
        for i, B in enumerate(self._B):
            if B.get_leg_labels() != self._B_labels:
                raise ValueError(
                    'B has wrong labels {0!r}, expected {1!r}'.format(
            ...
            if len(self._S[i2].shape) == 1:
                if (
                    ...
                    or self._S[i2].shape[0] != B.get_leg('vR').ind_len
                ):
    ...
    def from_lat_product_state(cls, lat, p_state, allow_incommensurate=False, **kwargs):
        ...
        **kwargs :
        ...
        if p_state.ndim == len(lat.shape):  # == lat.dim + 1
            ...
            p_state = to_array(
                p_state, shape=lat.shape + (None,), allow_incommensurate=allow_incommensurate
            )
        ...
        return cls.from_product_state(
            lat.mps_sites(), p_state_flat, **kwargs, unit_cell_width=lat.mps_unit_cell_width
        )
    ...
    def from_desired_bond_dimension(
        ...
        if bc == 'finite':
            ...
            SVs.append(SV / np.linalg.norm(SV))
            Bflat.append(
                np.random.rand(sites[-1].dim * chis[L - 2]).reshape(sites[-1].dim, B_vR, 1)
            )
            SVs = [np.ones(1)]
        elif bc == 'infinite':
            if isinstance(chis, int):
            ...
            raise NotImplementedError(
                'MPS.from_desired_bond_dimension not implemented for segment BC.'
            )
        psi = MPS.from_Bflat(
            sites,
            Bflat,
    ...
    def from_full(
        ...
        if legs_per_site > 1:
            psi = psi.combine_legs(
                [[1 + site * legs_per_site + n for n in range(legs_per_site)] for site in range(L)]
            )
        ...
        for local_psi, ind_map in zip(mps_covering, index_map):
            ...
            for site, i in zip(local_psi.sites, ind_map):
                if sites[i % L] is not None:
                    raise ValueError(
                        f'duplicate index {i:d} for {L:d}-site index_map\n{index_map!r}'
                    )
                sites[i % L] = site
    ...
    def get_SL(self, i):
        ...
        Takes care of shifting as described in :ref:`shift_symmetry`.
        """
        i_in_unit_cell, num_unit_cells = self._to_valid_bond_index(
    ...
    def overlap_translate_finite(self, psi, shift=1):
        ...
        for i in range(1, L):
            ...
            C = npc.tensordot(
                C,
                B_bra.conj(),
                axes=[['vR*'] + self._get_p_label(''), ['vL*'] + self._get_p_label('*')],
            )
    ...
    def enlarge_chi(self, extra_legs, random_fct=np.random.normal):
        ...
        for i, B in enumerate(self._B):
            ...
            if extra_leg_L is not None:
                ...
                extra_B = extra_B - npc.tensordot(
                    npc.tensordot(extra_B, B2.conj(), [1, 1]), B2, [1, 0]
                )
                ...
                extra_B, extra_R = npc.qr(
                    extra_B.itranspose([1, 0]), inner_qconj=-1, qtotal_Q=extra_B.qtotal
                )
                try:
                    ...
                    print(extra_R)
                    raise ValueError(
                        'QR for Gram-Schmidt messed up charges. ' 'Incompatible charges?'
                    ) from e
    ...
    def extract_segment(self, first, last):
        ...
        if remainder != 0:
            raise ValueError(
                f'Number of sites must be an integer multiple of {self.N_sites_per_hor_spacing}.'
            )
    ...
    def extract_enlarged_segment(
        ...
        if not new_first <= first < last <= new_last:
            raise ValueError(
                'expected new_first <= first < last <= new_last, but got '
        ...
        if (
            psi_left.bc == 'finite'
            and new_first == 0
            and psi_right.bc == 'finite'
            and new_last == psi_right.L - 1
        ):
            bc = 'finite'
    ...
    def entanglement_entropy_segment2(self, segment, n=1):
        ...
        rho = npc.tensordot(
            rho, rho.conj(), axes=(self._get_p_labels(1), self._get_p_labels(1, True))
        )
        not_in_segment = 0
    ...
    def entanglement_spectrum(self, by_charge=False):
        ...
        if by_charge:
            ...
            for i in range(self.L + 1)[self.nontrivial_bonds]:
                ...
                spectrum = [
                    (leg.get_charge(qi), np.sort(ss[leg.get_slice(qi)]))
    ...
    def get_rho_segment(self, segment):
        ...
        if len(segment) > 12:
            warnings.warn(
                "{0:d} sites in the segment, that's much!".format(len(segment)), stacklevel=2
            )
        if len(segment) > 20:
    ...
    def probability_per_charge(self, bond=0):
        ...
        if abs(np.sum(ps) - 1.0) > 1.0e-10:
            warnings.warn(
                'Probability_per_charge: Sum of probabilities not 1. Canonical form?', stacklevel=2
            )
    ...
    def get_charge_tree_for_given_charge_sector(sites: list, charge_sector: tuple):
        ...
        for i in reversed(range(L)):  # loop from right to left over all sites
            Q_R = np.array(
                list(Q_from_right[i + 1])
            )  # the dictionary of charges (Q_R) to the right of site i
            Q_L = set()
            ...
            for Q_p in sites[i].leg.charges:  # i.e. site.leg.charges=[[-1], [1]] for a SpinHalfSite
                ...
                Q_L_add = chinfo.make_valid(
                    Q_R - Q_p[np.newaxis]
                )  # from right to left in the tree we must subtract
        ...
        if tuple(charge_sector_left) not in Q_from_right[0]:
            raise ValueError(
                "can't get desired charge sector {charge_sector!r} "
        ...
        for i in range(L):
            ...
            for Q_p in sites[i].leg.charges:
                Q_R_add = chinfo.make_valid(
                    Q_L + Q_p[np.newaxis]
                )  # from left to right in the tree we must add
        ...
        assert (
            Q_from_left[-1] == Q_from_right[-1]
        ), "Left `charge_sector` doesn't meet the one on the right"
    ...
    def overlap(
        self, other, charge_sector=None, ignore_form=False, understood_infinite=False, **kwargs
    ):
    ...
    def expectation_value_terms_sum(self, term_list):
        ...
        mpo_graph = mpo.MPOGraph.from_terms(
            (ot, ct), self.sites, bc, unit_cell_width=self.unit_cell_width
        )
    ...
    def canonical_form_infinite2(
        self, renormalize=True, tol=1.0e-15, arnoldi_params=None, cutoff=1.0e-15
    ):
        ...
        new_As[0] = npc.tensordot(
            U.conj().ireplace_label('vR*', 'vL'), new_As[0], axes=['vL*', 'vL']
        )
    ...
    def _canonical_form_left_orthogonalize(self, L, tol, arnoldi_params):
        ...
        for _ in range(max_iters):
            ...
            TM = TransferMatrix.from_Ns_Ms(
                new_As, self._B, transpose=True, unit_cell_width=self.unit_cell_width
            )
    ...
    def _canonical_form_right_orthogonalize(self, R, tol, arnoldi_params):
        ...
        for _ in range(max_iters):
            ...
            TM = TransferMatrix.from_Ns_Ms(
                new_Bs, self._B, transpose=False, unit_cell_width=self.unit_cell_width
            )
    ...
    def _canonical_form_qr_L2R(self, L):
        ...
        for i in range(self.L):
            ...
            Q, L = npc.qr(
                LB, inner_labels=['vR', 'vL'], pos_diag_R=True, qtotal_Q=LB.qtotal, inner_qconj=+1
            )
            Qs[i] = Q.split_legs()
        return Qs, L
    ...
    def _canonical_form_qr_R2L(self, R):
        ...
        for i in reversed(range(self.L)):
            ...
            Q, R = npc.qr(
                BR, inner_labels=['vL', 'vR'], pos_diag_R=True, qtotal_Q=BR.qtotal, inner_qconj=-1
            )
            Qs[i] = Q.split_legs()
        return Qs, R
    ...
    def _correlation_length(self, target, tol_ev0, charge_sector, return_charges):
        ...
        if tol_ev0 is not None and abs(E[0] - 1.0) > tol_ev0:
            logger.warning(
                f'Correlation length: largest eigenvalue not one, but {E[0]:.12f}. '
    ...
    def add(self, other, alpha, beta, cutoff=1.0e-15):
        ...
        psi = self.__class__(
            self.sites, Bs, Ss, self.bc, form=None, unit_cell_width=self.unit_cell_width
        )  # new class instance
    ...
    def subspace_expansion(self, expand_into=[], trunc_par={'svd_min': 1.0e-8}):
        ...
        for j in reversed(range(1, L)):
            ...
            _, exact_B = npc.lq(
                current_Cs[0].combine_legs(['p', 'vR']), mode='reduced', inner_labels=['vR', 'vL']
            )
            eBhc_eB = npc.tensordot(
                exact_B.conj(), exact_B, axes=(['vL*'], ['vL'])
            )  # (p.vR)*, (p.vR)
            ...
            if npc.norm(proj) < 1.0e-12 or site_chi_max <= 0:
                ...
                if npc.norm(proj_rho) < 1.0e-12:
                    ...
                    w_enl_trunc, B_enl_trunc, err_trunc = eigh_rho(
                        proj_rho, trunc_par=site_trunc_par, sort='m>'
                    )
                    new_B = npc.concatenate(
                        [exact_B, B_enl_trunc.conj().transpose()], axis=0
                    ).split_legs()
                    eig_error += err_trunc
            ...
            self.set_SL(
                ...
                np.pad(
                    self.get_SL(j),
                    (0, new_B.get_leg('vL').ind_len - current_vL_dim),
                    mode='constant',
                    constant_values=0.0,
                ),
            )
    ...
    def apply_local_op(
        self, i, op, unitary=None, renormalize=False, cutoff=1.0e-13, understood_infinite=False
    ):
        ...
        if isinstance(op, str):
            ...
            if need_JW:
                if self.bc == 'infinite':
                    raise ValueError(
                        'open JW string ending in each unit cell' 'breaks translation invariance!'
                    )
                try:
                    ...
                    raise ValueError(
                        f'Would need JW string for operator {op!r}, '
                        "but can't extract JW signs from the charges"
                    ) from e
            opname = op
        else:
    ...
    def apply_local_term(self, term, autoJW=True, i_offset=0, canonicalize=True, renormalize=False):
        ...
        if has_extra_JW:
            ...
            try:
                ...
                raise ValueError(
                    f'Would need JW string for term {term!r}, '
                    "but can't extract JW signs from the charges"
                ) from e
        ...
        if isinstance(swap_op, str):
            ...
            if np.any(n_i * n_j):
                ...
                swap_op = npc.Array.from_ndarray(
                    np.diag(swap_op_diag).reshape([dL, dR, dL, dR]),
                    legs,
                    labels=['p1', 'p0', 'p0*', 'p1*'],
                )
    ...
    def compute_K(
        self, perm, swap_op='auto', trunc_par=None, canonicalize=1.0e-6, expected_mean_k=0.0
    ):
        ...
        logger.info(
            'compute_K: overlap %.5f, |o| = 1. - %.e5., trunc_err.eps=%.3e',
            ov[0],
            1.0 - np.abs(ov[0]),
            trunc_err.eps,
        )
    ...
    def _canonical_form_dominant_gram_matrix(self, bond0, transpose, tol_xi, guess=None):
        ...
        if len(eta) > 1:
            ...
            if xi > tol_xi:
                raise ValueError(
                    'Degenerate spectrum of TransferMatrix ' '(corr length xi={xi:.3e})'.format(
                        xi=xi
                    )
                )
        eta, G = eta[0], V[0]
    ...
    **init_env_data :
    ...
    def __init__(self, bra, ket, cache=None, **init_env_data):
        ...
        if not self.cache.long_term_storage.trivial and L < 8:
            warnings.warn(
                'non-trivial cache for short-length environment: '
    ...
    def init_first_LP_last_RP(
        self, init_LP=None, init_RP=None, age_LP=0, age_RP=0, start_env_sites=0
    ):
    ...
    def _check_compatible_legs(self, init_LP, init_RP, start_env_sites):
        ...
        if init_LP is not None:
            ...
            if incompatible_legs:
                msg = (
                    f'dropping `init_LP` with incompatible virtual legs: '
        ...
        if init_RP is not None:
            ...
            if incompatible_legs:
                msg = (
                    f'dropping `init_RP` with incompatible virtual legs: '
                    f'{", ".join(incompatible_legs)}'
                )
                warnings.warn(msg, stacklevel=2)
                init_RP = None
    ...
    def get_LP(self, i, store=True):
        ...
        for i0 in range(i, i - self.L, -1):
            i0_in_unit_cell, num_unit_cells = self._to_valid_site_index(
                i0, return_num_unit_cells=True
            )
...
class TransferMatrix(sparse.NpcLinearOperator):
    ...
    def __init__(
        self, bra, ket, shift_bra=0, shift_ket=0, transpose=False, charge_sector=0, form='B'
    ):
        L = lcm(bra.L, ket.L)
    ...
    def _init_from_Ns_Ms(
        ...
        assert (
            isinstance(unit_cell_width, int)
            and 0 < unit_cell_width <= L
            and L % unit_cell_width == 0
        )
        self.unit_cell_width = unit_cell_width
        assert len(ket_M) == L
...
class InitialStateBuilder:
    ...
    def run(self):
        ...
        if method is None:
            raise ValueError(
                f"initial state 'method'={method_name!r} not recognized in "
    ...
    def lat_product_state(self, p_state=None):
        ...
        psi = MPS.from_lat_product_state(
            self.lattice, p_state, dtype=dtype, allow_incommensurate=allow_incommensurate
        )
        return psi
    ...
    def mps_state_in_charge_sector(self, charge_sector=None, p_state=None):
        ...
        if p_state is None:
            p_state = np.ones(
                (lat.N_sites, sites[0].leg.block_number)
            )  # equal superposition product state
        self.check_filling(p_state)
        # sites must all be the same, however an error would be raised anyways in :meth:`project_onto_charge_sector`
        psi = MPS.project_onto_charge_sector(
            sites, p_state, charge_sector=charge_sector, dtype=dtype, bc=lat.bc_MPS
        )
        return psi
    ...
    def check_filling(self, p_state):
        ...
        if N_filled * q != N_total * p:  # int-version of N_filled/N_total != p/q
            raise ValueError(
                'unexpected filling {0:.5f} != check_filling = {1:.5f}'.format(
    ...
    def fill_where(self):
        ...
        p_state = np.where(
            fill_array,
            to_array([full], shape=shape, dtype=object),
