...
from ..tools.misc import find_subclass, inverse_permutation
    ...
    **site_ops :
    ...
    def change_charge(self, new_leg_charge=None, permute=None):
        ...
        if permute is not None:
            ...
            self.state_labels = dict(
                (lbl, int(inv_perm[i])) for lbl, i in self.state_labels.items()
            )
    ...
    def test_sanity(self):
        ...
        np.testing.assert_array_almost_equal(
            np.diag(np.exp(1.0j * np.pi * self.JW_exponent)), self.JW.to_ndarray(), 15
        )
    ...
    def add_op(self, name, op, need_JW=False, hc=None, permute_dense=None):
        ...
        if hc is None and name not in self.hc_ops:
...
def set_common_charges(sites, new_charges='same', new_names=None, new_mod=None, sort_charge=True):
    ...
    if isinstance(new_charges, str):
        if new_charges == 'same':
            ...
            new_charges = [
                [(1, s, i)] for s, site in enumerate(sites) for i in range(site.leg.chinfo.qnumber)
            ]
        else:
...
def kron(*ops, group=True):
    ...
    *ops : :class:`~tenpy.linalg.np_conserved.Array`
    ...
    product = npc.outer(
        ops[0].replace_labels(['p', 'p*'], ['p0', 'p0*']),
...
class SpinSite(Site):
    ...
    def __init__(self, S=0.5, conserve='Sz', sort_charge=True):
        ...
        ops = dict(Sp=Sp, Sm=Sm, Sz=Sz)
        if conserve == 'dipole':
            chinfo = npc.DipolarChargeInfo(
                [1, 1], ['2*Sz', 'dipole'], charge_idcs=[0], dipole_idcs=[1]
            )
...
class BosonSite(Site):
    ...
    def __init__(self, Nmax=1, conserve='N', filling=0.0):
        ...
        if conserve == 'dipole':
            chinfo = npc.DipolarChargeInfo(
                [1, 1], ['N', 'dipole'], charge_idcs=[0], dipole_idcs=[1]
            )
