...
# from .models.hubbard import (
#     BoseHubbardChain, BoseHubbardModel, FermiHubbardChain, FermiHubbardModel, FermiHubbardModel2
# )
...
estimate_simulation_RAM = None  # dummy
...
def _setup_arg_parser(width=None):
    ...
    def formatter(prog):
        return argparse.RawDescriptionHelpFormatter(prog, indent_increment=4, max_help_position=8, width=width)
    ...
    parser.add_argument(
        ...
        '-c',
        default=None,
        help="selects the Simulation (sub)class, e.g. 'GroundStateSearch' (default) or 'RealTimeEvolution'.",
    )
    parser.add_argument(
        '--merge',
        '-m',
        default='error',
        help="Selects how to merge conflicts in case of multiple yaml files. Options are 'error', 'first' or 'last'.",
    )
    parser.add_argument(
        '--RAM',
        action='store_true',
    ...
    opt_help = textwrap.dedent("""\
        ...
        argument.""")
    parser.add_argument('--option', '-o', nargs=2, action='append', metavar=('KEY', 'VALUE'), help=opt_help)
