...
class RealTimeEvolution(Simulation):
    ...
    def run_algorithm(self):
        ...
        while True:
            ...
            self.logger.info(
                f'total {self.engine.trunc_err} (only from truncation of Schmidt values)'
            )
...
class TimeDependentCorrelation(RealTimeEvolution):
    ...
    def init_measurements(self):
        ...
        if use_default_meas is False and connect_any_meas is None:
            warnings.warn(
                f'No measurements are being made, this might not make sense for {self.__class__}'
            )
...
class TimeDependentCorrelationEvolveBraKet(TimeDependentCorrelation):
    ...
    def init_algorithm(self, **kwargs):
        ...
        if resume_data_bra is not None:
            ...
            if 'psi' in resume_data_bra:
                self.psi_ground_state = resume_data_bra[
                    'psi'
                ]  # make sure to use resume data of bra
        ...
        self.engine_bra = AlgorithmClass(
            self.psi_ground_state, self.model, algorithm_params, **kwargs
        )
    ...
    def run_algorithm(self):
        while True:
            if np.real(self.engine.evolved_time) >= self.final_time:
                break
            self.logger.info(
                'evolve to time %.2f, max chi=%d', self.engine.evolved_time.real, max(self.psi.chi)
            )
    ...
    def get_resume_data(self) -> dict:
        ...
        resume_data = super(
            TimeDependentCorrelation, self
        ).get_resume_data()  # call Simulation's method
