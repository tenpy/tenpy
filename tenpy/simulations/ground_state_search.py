...
class PlaneWaveExcitations(GroundStateSearch):
    ...
    def __init__(self, options, *, gs_data=None, **kwargs):
        ...
        if 'group_sites' in self.options.keys():
            msg = 'No grouping allowed for Plane Wave through simulations since we cannot ungroup.'
    ...
    def load_groundstate(self):
        ...
        if np.linalg.norm(self.psi.norm_test()) > tol:
            if isinstance(self.psi, MPS):
                ...
                msg = 'uMPS does not pass norm test. Run VUMPS to get ground state or convert to MPS and canonicalize.'
                raise ValueError(msg)
        if isinstance(self.psi, MPS):
...
class OrthogonalExcitations(GroundStateSearch):
    ...
    def init_orthogonal_from_groundstate(self):
        ...
        if np.linalg.norm(psi0.norm_test()) > self.options.get('orthogonal_norm_tol', 1.0e-12, 'real'):
            self.logger.info('call psi.canonical_form() on ground state')
            psi0.canonical_form()
        if psi0.bc == 'infinite':
    ...
    def extract_segment_from_infinite(self, psi0_inf, model_inf, resume_data):
        ...
        if resume_data.get('converged_environments', False):
            ...
            guess_init_env_data = resume_data.get('init_env_data', None)
            H = model_inf.H_MPO
            env_data = MPOTransferMatrix.find_init_LP_RP(H, psi0_inf, first, last, guess_init_env_data)
        self.init_env_data = env_data
    ...
    def switch_charge_sector(self):
        ...
        self.logger.info('switch charge sector of the ground state [contracts environments from right]')
        apply_local_op = self.options.get('apply_local_op', None)
        switch_charge_sector = self.options.get('switch_charge_sector', None)
        ...
        if apply_local_op is not None:
            ...
            th0 = npc.Array.from_func(
                np.ones, [vL, vR], dtype=self.psi.dtype, qtotal=switch_charge_sector, labels=['vL', 'vR']
            )
            if th0.norm() == 0:
                raise ValueError(
                    f"Can't switch to desired charge sector at bond lef of site {i:d} with vL leg {vL:d} and vR {vR!r}"
                )
        ...
        if ground_state_energy > -1.0e-7:
            ...
            if (
                ...
                raise ValueError(
                    ...
                    f"lanczos_params['E_shift'] < {-2.0 * ground_state_energy:.2f}"
                )
...
class TopologicalExcitations(OrthogonalExcitations):
    ...
    def _extract_segment_from_finite(self, psi0_fin_alpha, psi0_fin_beta, model_fin):
        ...
        if first != 0 or last is not None:
            ...
            #self.init_env_data = self._contract_segment_boundaries(
            #     self.init_env_data, *ground_state_seg.segment_boundaries
            #     )
        else:
    ...
    def _glue_segments(self, seg_alpha, seg_beta, inf_alpha, inf_beta, model, indices):
        ...
        th0 = npc.Array.from_func(
            np.ones, [vL, vR], dtype=seg_alpha.dtype, qtotal=list(self.gluing_charge), labels=['vL', 'vR']
        )
        # See if lanczos_params is in yaml, if not use empty dictionary
        lanczos_params = self.options.get('lanczos_params', {})
        _, th0, _ = krylov_based.LanczosGroundState(H0, th0, lanczos_params).run()

        norm = npc.norm(th0)
        self.logger.info('Norm of theta guess: %.8f', norm)
        if np.isclose(norm, 0):
            msg = f'Norm of inserted theta with charge {list(self.gluing_charge)} on site index {boundary:d} is zero.'
            raise ValueError(msg)

        U, s, Vh = npc.svd(th0, inner_labels=['vR', 'vL'])
        # Put AU into last site of left segment
        seg_alpha.set_B(
            seg_alpha.L - 1, npc.tensordot(seg_alpha.get_B(seg_alpha.L - 1, 'A'), U, axes=['vR', 'vL']), form='A'
        )
        seg_alpha.set_SR(seg_alpha.L - 1, s)
        # Put Vh B into first site of right segment
        seg_beta.set_B(0, npc.tensordot(Vh, seg_beta.get_B(0, 'B'), axes=['vR', 'vL']), form='B')
        seg_beta.set_SL(0, s)
    ...
    def _concatenate_segments(self, seg_alpha, seg_beta, inf_alpha):
        ...
        cp = MPS(
            l_sites + r_sites, lA + rB, lS + rS, 'segment', ['A'] * seg_alpha.L + ['B'] * seg_beta.L, inf_alpha.norm
        )
    ...
    def get_reference_energy(self, psi0_alpha, psi0_beta):
        ...
        if psi0_alpha.finite:
            ...
            correction = (
                self.correction(psi0_alpha, psi0_beta, env_alpha, env_beta, last).real / psi0_alpha.L
                - eta_R_beta
                + eta_R_alpha
            )
