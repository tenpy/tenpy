...
def m_simulation_parameter(
    results, psi, model, simulation, recursive_key, results_key=None, **kwargs
):
...
def m_energy_MPO(results, psi, model, simulation, results_key='energy_MPO'):
    ...
    psi = (
        simulation.psi
    )  # take original psi, possibly grouped, but compatible with simulation.model
    if psi.bc == 'segment':
...
def m_onsite_expectation_value(
    results, psi, model, simulation, opname, results_key=None, fix_u=None, **kwargs
):
...
def m_correlation_length(
    results, psi, model, simulation, results_key='correlation_length', unit=None, **kwargs
):
