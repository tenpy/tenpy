...
.. todo :
    `SpinChain` is not implemented yet...
.. todo :
    test...
"""
...
from .model import TwoSiteModel, NearestNeighborModel, MPOModel
from ..linalg import np_conserved as npc


class XXZChain(TwoSiteModel, NearestNeighborModel, MPOModel):
    ...
    def __init__(self, L=2, Jxx=1., Jz=1., hz=0., bc_MPS='finite'):
        ...
        leg = npc.LegCharge(npc.ChargeInfo([1], ['2*Sz']), [1, -1])
        # 2) onsite operators
        Sp = [[0., 1.], [0., 0.]]
        Sm = [[0., 0.], [1., 0.]]
        Sz = [[0.5, 0.], [0., -0.5]]
        ...
        # 5) initialize TwoSiteModel
        TwoSiteModel.__init__(self, lat, bc_coupling)
        # 6) add terms of the Hamiltonian
        # (u is always 0 as we have only one site in the unit cell)
        self.add_onsite(hz, 0, 'Sz')
        Jxx_half = np.asarry(Jxx)*0.5  # convert to array: allow `array_like` Jxx
        self.add_coupling(Jxx_half, 0, 'Sp', 0, 'Sm', 1)
        self.add_coupling(Jxx_half, 0, 'Sm', 0, 'Sp', 1)
        self.add_coupling(Jz, 0, 'Sz', 0, 'Sz', 1)
        # 7) calculate MPO
        MPOModel.__init__(self, lat, self.calc_H_MPO())
        # 8) initialize H_MPO     (the order of 7/8 doesn't matter)
