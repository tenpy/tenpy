...
def main():
    ...
    parser.add_argument('--debug', help='run with lower bond dimensions and fewer g values',
                        action='store_true')
    parser.add_argument(
    ...
    parser.add_argument(
        '-c', type=str, default='None', help='What should be conserved'
    )
    parser.add_argument(
    ...
    if args.debug:
        test_local(outfolder, conserve)
        return
    
    if args.debug_plot:
        test_plotting(outfolder, conserve)
        return
    
    g_list = np.linspace(2, 4, 101, endpoint=True)
    chi_list = [32, 128, 512]
    Ly_list = [4, 6, 8]
    res = simulate(chi_list=chi_list, Ly_list=Ly_list, g_list=g_list,
                   logfile=os.path.join(outfolder, f'tfi_2D_{conserve}.log'),
                   results_file=os.path.join(outfolder, f'tfi_2D_{conserve}.pkl'))
    plot_results(res, plot_file=os.path.join(outfolder, f'tfi_2D_{conserve}.pdf'))
...
def test_local(outfolder: str, conserve):
    ...
    chi_list = [20, 30, 50]
    Ly_list = [4, 6]
    res = simulate(chi_list, Ly_list, g_list, conserve=conserve,
                   logfile=os.path.join(outfolder, f'tfi_2D_{conserve}.log'),
                   results_file=os.path.join(outfolder, f'tfi_2D_{conserve}.pkl'))
    plot_results(res, plot_file=os.path.join(outfolder, f'tfi_2D_{conserve}.pdf'), show=True)
...
def test_plotting(outfolder: str, conserve):
    ...
    res_file = os.path.join(outfolder, f'tfi_2D_{conserve}.pkl')
    with open(res_file, 'rb') as f:
        results = pickle.load(f)
    plot_results(results, os.path.join(outfolder, f'test_plotting_tfi_2D_{conserve}.pdf'), show=True)


def simulate(chi_list, Ly_list, g_list, conserve, logfile: str, results_file=None):
...
def sweep_phase_diagram(g_list, conserve, chi: int, Ly: int):
    ...
    return dict(
        g_list=g_list, energy_list=energy_list, entropies=entropies, mag_z_list=mag_z_list,
...
def single_run(g: float, Ly: int, conserve, chi_max: int, psi=None):
    ...
    mag_z = np.average(psi.expectation_value('Sz'))
    mag_x = np.average(psi.expectation_value('Sx'))
    corr_xx = psi.correlation_function('Sx', 'Sx', [0], [10 * model.lat.N_sites]).item()
...
def plot_results(results, plot_file=None, show=False):
    ...
    axs[0, 0].set_ylabel(r'$\langle Z \rangle$')
    for key in results:
