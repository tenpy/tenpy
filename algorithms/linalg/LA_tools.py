...
def lanczos(A,
    ...
    while (((p_err > max_p_err or Delta_e0 > max_Delta_e0) and k < N_max) or
           (k < N_min)) and above_ulp:
...
def svd_theta(theta, truncation_par, tol=0., chargeR=None):
    ...
    if 'tol' in truncation_par:
        tol = truncation_par['tol']  # override tol if it's in the dictionary
    X, Y, Z = npc.svd(theta,
                      full_matrices=False,
                      compute_uv=True,
                      cutoff=1e-16,
