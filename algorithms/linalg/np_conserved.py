...
# Definition:
# bint array_equiv_mod_q(np.ndarray[c_int, ndim=1] a1, np.ndarray[c_int,
# ndim=1] a2, np.ndarray[c_int, ndim=1] mod_q)
...
class array:
    ...
    def __init__(self, rank, dtype=float64):
        ...
        # TODO call d rank
        # dimension (number of legs), in np.ndarray this is ndim
        self.rank = rank
        # np.array( (rank,), dtype = np.intp) total shape tuple or ndarray =
        # (n_0, ..., n_{d-1})
        self.shape = None
        # np.int number of conserved charges (applies globally), shoulde be >=
        # 0.
        self.num_q = None
        # np.array( (num_q, ) dtype = np.int) 1-array of mod-q
        self.mod_q = None
        # np.array( (num_q,), dtype = np.int ) total charge of tensor, inward
        # pointing convention
        self.charge = None
        # [ np.ndarray( (num_block, 2 + num_q), dtype = np.int) ] List[d] of 2-arrays. Each array, qi_{m, n}, is such that m labels charges sectors, qi_{m, 0} is the starting index of mth charge sector, qi_{m, 1} is ending index, and q_{m, 2:} is charge of mth sector
        self.q_ind = None
        # 1-array shaped (d,), which notes if the corresponding q_ind is
        # charge-conjugated (values +-1)
        self.q_conj = None
        # np.ndarray( (num_dat, d), dtype = np.uint) 2-array, of the q-index
        ...
        self.sorted = False
        # { x:x for x in range(rank)} # Dict takes labels -> index numbers
        self.labels = None
    ...
    # rank
        ...
        if rank < 0:
            raise RuntimeError, "rank is not positive: rank = " + str(rank)
        if rank == 0:
            raise NotImplementedError, "rank is zero"
    # shape
        shape = self.shape
    ...
    # num_q
        num_q = self.num_q
        if not isinstance(num_q, int):
            raise RuntimeError, "num_q is not an int (type=" + str(type(num_q)) + ")"
        if num_q < 0:
            raise RuntimeError, "negative num_q: " + str(num_q)
    # mod_q
        mod_q = self.mod_q
    ...
    ##  q_ind & q_conj
        ...
        for l in range(rank):
            ...
            if not isinstance(tbl, np.ndarray):
                raise RuntimeError, "q_ind[" + str(l) + "] not a numpy.ndarray"
            if tbl.dtype != int:
                raise RuntimeError, "q_ind[%s].dtype not int (type %s)" % (l, type(tbl.dtype))
            if tbl.ndim != 2 or tbl.shape[1] != num_q + 2:
                raise RuntimeError, "q_ind[" + str(l) + "] has incorrect shape: " + str(tbl)
    ...
    ##  q_dat & dat
        ...
        dat = self.dat
        dtype = self.dtype
        if not isinstance(q_dat, np.ndarray):
            raise RuntimeError, "q_dat not a numpy.ndarray"
        if q_dat.dtype != np.uint:
            raise RuntimeError, "q_dat.dtype not np.uint (%s)" % q_dat.dtype
        if q_dat.ndim != 2:
        ...
        for i in range(len(dat)):
            q_row = q_dat[i]
            T_row = dat[i]
            if not isinstance(T_row, np.ndarray):
                raise RuntimeError, "dat[" + str(i) + "] not a numpy.ndarray: " + str(type(T_row))
            if T_row.dtype != dtype:
                warningMsg = "dtype mismatch: dat[%s].dtype = %s, self.dtype = %s" % (
                    i, T_row.dtype, dtype)
                raise RuntimeError, warningMsg
            if anynan(T_row):
...
Modifies self (done in-place)
    ...
    def detect_ndarray_charge(cls, a, q_ind, q_conj=None, mod_q=None):
        ...
        for qs in itertools.product(*[xrange(len(q_ind[i])) for i in reversed(xrange(d))]):
            ...
            # slices corresponding to qs
            sl = tuple([slice(s[0], s[1]) for s in sector])
...
# Bunch / Not bunch
    ...
    def from_ndarray_trivial(cls, a, q_conj=None):
        ...
        rank = a.ndim
        if rank == 0:
            raise ValueError, "len(a.shape) = 0"
        if np.any(a.shape == 0):
            raise ValueError, "a.shape has a 0: " + str(a.shape)
        ac = cls(rank, a.dtype)
...
##########################################################################
    ...
    def stride_order(self):
        ...
        if len(self.dat) > 0:
            ...
            # st may have duplicate strides (if dim = 1 for a particular leg),
            # so vstacking np.arange makes lexsort stable
    ...
    def get_q_index(self, leg, i):
        ...
        if i < 0 or i >= self.shape[leg]:
            raise ValueError, "Out of range: shape = %s, leg %s, i = %s" % (self.shape, leg, i)
...
#       pass
    ...
    def split_legs(self, legs, leg_pipes, verbose=0, timing=False):
        ...
        for l in range(nsplit):
            ...
            a.q_conj[s_map[l]] = leg_pipes[l].q_conj * \
                leg_pipes[l].qt_conj * self.q_conj[legs[l]]
        ...
        if self.labels is not None:
            ...
            for s, d in u_map:  # Un split segments: slices src->dest
                ...
                for v in xrange(s.start, s.stop):
                    # and rev[v][0]!='?': #If it has label, add and translate
                    # index
                    if v in rev:
        ...
        for r in xrange(len(self.q_dat)):       # scan q_dat's rows
            ...
            # scan every charge sub sector that is contained in oldrow
    ...
    def remove_Q(self, Q, fracture=True):
        ...
        """
                ...
                """
        Q = tonparray(Q)
        if np.any(Q < 0) or np.any(Q >= self.num_q):
            raise ValueError, "Q not between 0 and " + str(self.num_q) + ": " + str(Q)
    ...
    def permute_Q(self, Qorder):
        ...
        if Qorder.ndim > 1:
            raise ValueError
        if len(Qorder) != self.num_q:
            raise ValueError
        if np.any(np.argsort(np.argsort(Qorder)) != Qorder):
            raise ValueError, (Qorder, np.argsort(Qorder), np.argsort(np.argsort(Qorder)))
        a = self.copy()
        a.charge = a.charge[Qorder]
        a.mod_q = a.mod_q[Qorder]
        # keep the first two, and swap the last num_q charges
        Qind_order = np.array([0, 1] + [q + 2 for q in Qorder])
    ...
    def ibinary_blockwise(self, func, b):
        ...
        # If the q_dat structure is identical, we can immediately run through
        # the data.
        if Na == Nb and np.array_equiv(aq, bq):
            self.dat = [func(*((at, bt) + args), **keywords)
                        for at, bt in itertools.izip(adat, bdat)]
    ...
    def iscale_axis(self, s, axis=-1):
        ...
        if axis != -1:
            self.dat = [np.swapaxes(np.swapaxes(t, axis, -1) * s[q_i[q, 0]:q_i[q, 1]], axis, -1)
                        for q, t in itertools.izip(self.q_dat[:, axis], self.dat)]
        else:  # not sure if this is faster

            self.dat = [t * s[q_i[q, 0]:q_i[q, 1]]
                        for q, t in itertools.izip(self.q_dat[:, axis], self.dat)]
    ...
    def scale_axis(self, s, axis=-1):
        ...
        if axis != -1:
            b.dat = [np.swapaxes(np.swapaxes(t, axis, -1) * s[q_i[q, 0]:q_i[q, 1]], axis, -1)
                     for q, t in itertools.izip(self.q_dat[:, axis], self.dat)]
        else:  # not sure if this is faster
            # try:
            b.dat = [t * s[q_i[q, 0]:q_i[q, 1]]
                     for q, t in itertools.izip(self.q_dat[:, axis], self.dat)]
            # except:
            #   self.check_sanity()
            #   print self.dat
    ...
    def ipermute_qi(self, axis, perm):
        ...
        if len(perm) != len(qind):
            raise ValueError
        # temporarily store the size of each q-block in the 0th column
        qind[:, 0] = qind[:, 1] - qind[:, 0]
        qind = qind[perm, :]
        # figure out the q_ind from the sizes
        qind[:, 1] = np.cumsum(qind[:, 0])
        qind[1:, 0] = qind[:-1, 1]
        qind[0, 0] = 0
...
class leg_pipe:
    ...
    def __init__(self, legs):
        ...
        # l2, ...).  Equivalent to len(q_ind[i])
        self.qshape = None
        # Number of blocks in new combined leg, tuple  (l,).  Equivalent to
        # len(qt_ind)
        self.t_qshape = None

        # Size in each individual leg, represented as 1-array (l1, l2, ...).
        # Equivalent to q_ind[i][-1,1].
        self.shape = None
        # Size of the combined leg, np.int64.  Equivalent to qt_ind[-1,1] =
        # product of shape.
        self.t_shape = None
        ...
        # permutation array from qt_map to q_map (size # block)
        self.perm = None
        # a 2-array that takes total charge sectors to slices in qt_map, shape
        # (t_qshape, 2)
    ...
    # rank
        ...
        if nlegs <= 0:
            raise ValueError, "nlegs is negative: " + str(nlegs)
    # mod_q
        mod_q = self.mod_q
        if not isinstance(mod_q, np.ndarray):
            raise ValueError, "mod_q not a numpy.ndarray (type=%s)" % str(type(mod_q))
        if mod_q.ndim != 1:
            raise ValueError, "mod_q is not 1D: shape = " + str(mod_q.shape)
        if mod_q.dtype != int:
    ...
    def make_pipe(cls, q_ind, q_conj, qt_conj=1, block_single_legs=True, mod_q=None, verbose=0):
        ...
        # Single legs get processed (combining blocks with the same charges),
        # unless block_single_legs = False
        ...
        qshape = pipe.qshape = tuple([len(q) for q in q_ind])
        pipe.shape = shape_from_q_ind(q_ind)
        ...
        # now qt_map[:, 2:2+d] has a list of all possible sub q-indices
        qt_map = qt_map.reshape((size, -1)).transpose()
        ...
        # Save order needed to bring back mult_ind order
        pipe.perm = np.argsort(revperm)
        ...
        # chop qt_map back down to the specification
        qt_map = qt_map[:, 0:3 + d]
...
def tensordot_compat(a, b, axes=2, suppress_warning=False, verbose=0):
    ...
    # axes_a,b are now each list (same len) of indices to contract (na,b are
    ...
    if a.num_q != b.num_q:
        raise ValueError, "num_q mismatch"
    if np.any(a.mod_q != b.mod_q):
...
def trace(a, axis1=0, axis2=1):
    ...
    # np.not_equal( np.arange(a.rank), axis2) ) #True where legs survive trace
...
def pinv(a, rcond=1e-15):
    ...
    if isinstance(a, np.ndarray):
        if a.ndim == 1:
            if rcond != 0.:
                ...
                # take inverse
                return np.choose(np.abs(a) < rcond, [a**(-1), np.zeros_like(a)])
            else:
                return a**(-1)
        else:
    ...
    if is_blocked == False:
        # The 'outer' facing legs are permuted back. Reverse order of pipes!
        v = v.split_legs([0, 1], pipes[::-1])
...
def svd(a, full_matrices=0, compute_uv=1, overwrite_a=False, cutoff=None, chargeR=None, inner_labels=[None, None]):
    ...
    U.q_dat = []
    V.q_dat = []
    # TODO The order here determines q_ind, pick convention?
    for q, t in itertools.izip(a.q_dat, a.dat):
        ...
        if num > 0:
            ...
            if compute_uv:
                ...
                # q_dats entries of U, V identical to a (except for dropped
                # sectors)
...
def eigh(a, UPLO='L', sort=None, debug=False):
    ...
    if len(a.shape) != 2 or a.shape[0] != a.shape[1]:
        raise ValueError, "npc.eigh():  Non-square matrix with shape = {}.".format(a.shape)
    assert q_ind_contractible(
        a, 0, a, 1, uncontractible_error_msg="npc.eigh():  Matrix legs mismatch.")
...
def eig(a, sort=None, debug=False):
    ...
    if len(a.shape) != 2 or a.shape[0] != a.shape[1]:
        raise ValueError, "npc.eig():  Non-square matrix with shape = {}.".format(a.shape)
    assert q_ind_contractible(
        a, 0, a, 1, uncontractible_error_msg="npc.eig():  Matrix legs mismatch.")
...
def eigvalsh(a, UPLO='L', sort=None, debug=False):
    ...
    """
    if len(a.shape) != 2 or a.shape[0] != a.shape[1]:
        raise ValueError, "npc.eigvalsh():  Non-square matrix with shape = {}.".format(a.shape)
    assert q_ind_contractible(
        a, 0, a, 1, uncontractible_error_msg="npc.eigvalsh():  Matrix legs mismatch.")
...
def eigvals(a, sort=None, debug=False):
    ...
    """
    if len(a.shape) != 2 or a.shape[0] != a.shape[1]:
        raise ValueError, "npc.eigvals():  Non-square matrix with shape = {}.".format(a.shape)
    assert q_ind_contractible(
        a, 0, a, 1, uncontractible_error_msg="npc.eigvals():  Matrix legs mismatch.")
...
def speigs(a, charge_sector, k=6, M=None, sigma=None, which='LM', v0=None, ncv=None, maxiter=None, tol=0, return_eigenvectors=True, Minv=None, OPinv=None, OPpart=None):
    ...
    assert q_ind_contractible(
        a, 0, a, 1, uncontractible_error_msg="npc.speigs():  Matrix legs mismatch.")
    ...
    # TODO - should actually check if charge sector exists, and return
    # appropriate null eigensystem if present or raise error if not.
    if not found:
