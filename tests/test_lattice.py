...
def test_lattice():
    ...
    for order in ['default', 'Fstyle', 'snake', 'snakeFstyle']:
        print('order =', order)
        Ls = [5, 2]
        basis = [[1.0, 1.0], [0.0, 1.0]]
        pos = [[0.1, 0.0], [0.2, 0.0]]
        lat = lattice.Lattice(
            Ls, [site1, site2], order=order, basis=basis, positions=pos, bc='periodic', bc_MPS='infinite'
        )
        ...
        # test position
        npt.assert_equal([4.1, 5.0], lat.position(idx))
        ...
        for u in range(len(lat.unit_cell)):
            ...
            A_u_res_masked = lat.mps2lat_values_masked(A_u, axes=[-1, 0], mps_inds=[mps_u] * 2, include_u=[False] * 2)
...
def test_IrregularLattice():
    ...
    ir = lattice.IrregularLattice(
        reg, [[1, 1, 0], [1, 1, 1], [0, 0, 0]], ([[1, 1, 2], [1, 1, 3]], [7, 10]), [s2, s2], [[-0.1, 0.0], [0.1, 0.0]]
    )
...
def test_HelicalLattice():
    ...
    p_state = [[['up', 'down'], ['down', 'down'], ['up', 'down']], [['down', 'down'], ['up', 'down'], ['down', 'down']]]
...
def test_pairs():
    ...
    for lat in lattices:
        ...
        found_dist_pairs = lat.find_coupling_pairs(5, 3.0)
...
def test_possible_couplings():
    lat_reg = lattice.Honeycomb(2, 3, [None, None], order='snake', bc='periodic', bc_MPS='infinite')
...
def test_index_conversion():
    ...
    for order in ['snake', 'default']:
        ...
        npt.assert_equal(sz1_lat[:, :, 0], +1.0)
        npt.assert_equal(sz1_lat[:, :, 1], -1.0)
        # and a random state
        ...
        expect_sz2 = np.sum(state2**2 * np.array([1.0, -1]), axis=-1)
