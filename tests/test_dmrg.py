...
def test_dmrg_rerun(L=2):
    ...
    M = TFIChain(model_params)
    psi = mps.MPS.from_product_state(
        M.lat.mps_sites(), [0] * L, bc=bc_MPS, unit_cell_width=M.lat.mps_unit_cell_width
    )
...
def test_dmrg_excited(eps=1.0e-12):
    ...
    # first DMRG run
    psi0 = mps.MPS.from_product_state(
        M.lat.mps_sites(), [0] * L, bc=bc, unit_cell_width=M.lat.mps_unit_cell_width
    )
    dmrg_pars = {
        'N_sweeps_check': 1,
        'lanczos_params': {'reortho': False},
        'diag_method': 'lanczos',
        'combine': True,
    }
...
def test_dmrg_dipole_conservation(N, bc_MPS, S=1, tol=1.0e-13, J4=0.0):
    ...
    ):  # may issue warning that H is zero in sector. thats ok since we use a mixer.
        warnings.simplefilter('ignore')
        M_dip = DipolarSpinChain(
            dict(L=2 * N, S=S, J3=1.0, J4=J4, bc_MPS=bc_MPS, conserve='dipole')
        )
...
def test_dmrg_mixer_cleanup(L, bc_MPS):
    ...
    print('Checking consistency of old environments...')
    ...
    print('Checking consistency of new environments...')
    for i in range(L):
        assert np.allclose(engine.env.full_contraction(i), old_contractions[i])

    print('Check that expectation values have not changed...')
    for op in ['Sx', 'Sz']:
...
def test_segment_dmrg(model):
    ...
    assert np.allclose(
        psi1_segment.entanglement_entropy(), np.mean(psi0_infinite.entanglement_entropy())
    )
    assert np.allclose(
        psi0_infinite.expectation_value('Sz', [0]), psi1_segment.expectation_value('Sz', [0])
    )
    assert np.allclose(
        psi0_infinite.expectation_value('Sx', [0]), psi1_segment.expectation_value('Sx', [0])
    )
