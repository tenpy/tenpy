...
def test_purification_mps():
    for L in [4, 2, 1]:
        ...
        npt.assert_allclose(N, np.ones([L]), atol=1.0e-13)
        E = psi.expectation_value('Sz')
        npt.assert_allclose(E, np.zeros([L]), atol=1.0e-13)
        C = psi.correlation_function('Sz', 'Sz')
        npt.assert_allclose(C, 0.5 * 0.5 * np.eye(L), atol=1.0e-13)
        coords, mutinf = psi.mutinf_two_site()
        for (i, j), Iij in zip(coords, mutinf):
            print(repr((i, j)), Iij)
        if L > 1:
            assert np.max(np.abs(mutinf)) < 1.0e-14
        if L >= 2:
...
def test_canonical_purification(conserve_ancilla, L=6, charge_sector=0, eps=1.0e-14):
    site = spin_half
    ...
    assert abs(sum(Szs) - charge_sector) < 1.0e-13
    ...
    xxz_pars = dict(L=L, Jxx=1.0, Jz=3.0, hz=0.0, bc_MPS='finite', sort_charge=False)
    # sort_charge should be same as for global `spin_half`.
    M = XXZChain(xxz_pars)
    TEBD_params = {
        'trunc_params': {'chi_max': 16, 'svd_min': 1.0e-8},
...
def test_purification_TEBD(L=3):
    xxz_pars = dict(L=L, Jxx=1.0, Jz=3.0, hz=0.0, bc_MPS='finite', sort_charge=True)
    M = XXZChain(xxz_pars)
...
def test_purification_MPO(L=6):
    xxz_pars = dict(L=L, Jxx=1.0, Jz=2.0, hz=0.0, bc_MPS='finite', sort_charge=True)
    M = XXZChain(xxz_pars)
    psi = purification_mps.PurificationMPS.from_infiniteT(
        M.lat.mps_sites(), bc='finite', unit_cell_width=M.lat.mps_unit_cell_width
    )
    options = {'trunc_params': {'chi_max': 50, 'svd_min': 1.0e-8}}
...
def test_renyi_disentangler(L=4, eps=1.0e-15):
    xxz_pars = dict(L=L, Jxx=1.0, Jz=3.0, hz=0.0, bc_MPS='finite', sort_charge=True)
    M = XXZChain(xxz_pars)
    psi = purification_mps.PurificationMPS.from_infiniteT(
        M.lat.mps_sites(), bc='finite', unit_cell_width=M.lat.mps_unit_cell_width
    ...
    U0 = npc.outer(npc.eye_like(theta, 'q0', labels=['q0', 'q0*']), npc.eye_like(theta, 'q1', labels=['q1', 'q1*']))
    U = U0
    Sold = np.inf
    for i in range(20):
...
def gen_disentangler_psi_singlet_test(site_P=spin_half, L=6, max_range=4):
    ...
    print('entanglement entropy: ', psi0.entanglement_entropy() / np.log(2.0))
    ...
    tebd_pars = dict(trunc_params={'trunc_cut': 1.0e-10}, disentangle='diag')
    ...
    assert np.all(mutinf_Q < 1.0e-10)
...
def test_purification_apply_local_op(n_sites, act_on_p, act_on_q):
    ...
    psi_inf = purification_mps.PurificationMPS.from_infiniteT(sites=model.lat.mps_sites(), unit_cell_width=8)
    psi = psi_inf.copy()
    ...
    print(f'{op_labels=}')
    op = npc.Array.from_func(
        np.random.random,
        op_legs + [l.conj() for l in op_legs],
        qtotal=None,
        shape_kw='size',
        labels=op_labels + [f'{l}*' for l in op_labels],
    )
...
def test_purification_from_density_matrix(n_sites, conserve):
    ...
    A = npc.Array.from_func(
        np.random.random,
        p_legs + [l.conj() for l in p_legs],
        qtotal=None,
        shape_kw='size',
        labels=p_labels + p_conj_labels,
    )
    # make hermitian
    ...
    psi = purification_mps.PurificationMPS.from_density_matrix(sites=[s] * n_sites, rho=rho, unit_cell_width=n_sites)
    psi.test_sanity()
