...
def test_bosonic_model_TEBD():
    ...
    L = 15
    model = mods.hubbard.BoseHubbardChain(
        {'conserve': None, 'U': 1, 't': 1, 'bc_MPS': 'finite', 'L': L, 'n_max': 4}
    )
    # Test TEBD first
    psi = mps.MPS.from_product_state(
        model.lat.mps_sites(), [0] * L, unit_cell_width=model.lat.mps_unit_cell_width
    )
    ...
    assert (
        abs(estimate - exact) < 1e-10
    ), f'TEBD RAM did not match expectation (expected: {exact}, gotten: {estimate})'
...
def test_bosonic_model_DMRG():
    ...
    estimate = engine.estimate_RAM()
    assert abs(estimate - exact) < 1.0e-10, (
