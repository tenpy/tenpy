...
def test_exp_decaying_terms():
    ...
    H1 = mpo.MPOGraph.from_term_list(
        ts, sites, bc='finite', unit_cell_width=L
    ).build_MPO()  # Chain geometry
    ...
    edt = ExponentiallyDecayingTerms(L)
    p, l = 3.0, 0.5
    edt.add_exponentially_decaying_coupling(
        p, l, 'X', 'Y', subsites=[1, 3, 5, 7], subsites_start=[0, 2, 4, 6]
    )
    edt._test_terms(sites)
    ts = edt.to_TermList(bc='finite', cutoff=0.01)
    ts_desired = [
    ...
    p, l = 3.0, 0.5
    edt.add_exponentially_decaying_coupling(
        p, l, 'X', 'Y', subsites=[0, 2, 4, 6], subsites_start=[1, 3, 5, 7]
    )
    edt._test_terms(sites)
    ts = edt.to_TermList(bc='finite', cutoff=0.01)
    ts_desired = [
...
def test_exp_non_uniform_decaying_terms_subsites_start(bc):
    ...
    l = 1.0 / (1 + np.arange(L))
    edt.add_exponentially_decaying_coupling(
        p, l, 'X', 'Y', subsites=subsites, subsites_start=subsites_start
    )
    edt._test_terms(sites)
