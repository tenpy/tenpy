...
def helper_test_init_env(psi, E, H, name, tol=1e-10):
    ...
    assert npc.norm(env_base['init_LP'] - init_env['init_LP']) < 1e-8, (
        name + ': LP_iterative not converged'
    )
    assert npc.norm(env_base['init_RP'] - init_env['init_RP']) < 1e-8, (
        name + ': RP_iterative not converged'
    )
    assert abs(E_iter[0] - E_base[0]) < tol
    assert abs(E_iter[1] - E_base[1]) < tol, (
        name + ": Energies of iterative environment intialization don't match"
    )
...
def helper_test_H_square(psi, square_of_H, H, name, state):
    ...
    def var(e1, e2, rhoL, rhoR):
        ...
        def check_res(res, max_pow, env_name='env1'):
            combs = [(j, jj) for j in range(1, max_pow + 1) for jj in range(j)] + [
                (j, j) for j in range(max_pow + 1)
            ]
            for c in combs:
                ...
                if c[0] + c[1] > max_pow:
                    assert abs(entries[0]) < 1e-10, (
                        name + ': Higher power environments have overlap for: ' + env_name
                    )
        ...
        for jL, eL in enumerate(Ls1):
            for jR, eR in enumerate(Rs1):
                res_1[(jL, jR)] = [
                    npc.tensordot(
                        eL, e1['init_RP'][jR], axes=[['vR', 'wR', 'vR*'], ['vL', 'wL', 'vL*']]
                    ),
                    npc.tensordot(
                        e1['init_LP'][jL], eR, axes=[['vR', 'wR', 'vR*'], ['vL', 'wL', 'vL*']]
                    ),
                ]
        check_res(res_1, 1)
        ...
        for jL, eL in enumerate(Ls):
            for jR, eR in enumerate(Rs):
                res_2[(jL, jR)] = [
                    npc.tensordot(
                        eL, e2['init_RP'][jR], axes=[['vR', 'wR', 'vR*'], ['vL', 'wL', 'vL*']]
                    ),
                    npc.tensordot(
                        e2['init_LP'][jL], eR, axes=[['vR', 'wR', 'vR*'], ['vL', 'wL', 'vL*']]
                    ),
                ]
    ...
    assert (npc.norm(LP_diff)) < 1e-8, (
        name + 'Left environment of H**2 is not the correct eigenvector'
    )
...
def setup(test_case: int):
    ...
    if test_case == 0 or test_case == 2:
        ...
        eng = dmrg_eng(
            psi_tfi, m_tfi, {'min_sweeps': 19, 'max_sweeps': 29, 'trunc_params': {'chi_max': 32}}
        )
    ...
    if test_case == 1:
        ...
        # exp_decaying
        m_couple.add_exponentially_decaying_coupling(
            1.2, 0.42, 'Sigmaz', 'Sigmaz', subsites=[1, 4, 7, 10]
        )
        m_couple.add_exponentially_decaying_coupling(
            0.32, 0.42, 'Sigmaz', 'Sigmaz', subsites=[0, 3, 6, 9]
        )
        # onsite
        ...
        eng = dmrg_eng(
            psi_couple,
            m_couple_MPO,
            {'min_sweeps': 19, 'max_sweeps': 29, 'trunc_params': {'chi_max': 8}},
        )
...
def test_init_LP_RP_iterative(test_case):
    ...
    for sort_charges in [0, 1, 2]:
        ...
        assert H._outer_permutation[-1] == H.IdR[-1], (
            name + ': wrong IdR index in _outer_permutation'
        )
        # IdL, IdR cycle
