...
def test_MPO_conversion():
    ...
    for i in range(L):
        ...
        s.add_op('X_{i:d}'.format(i=i), np.array([[0.0, 1.0], [1.0, 0.0]]))
        s.add_op('Y_{i:d}'.format(i=i), np.array([[0.0, 1.0], [-1.0, 0.0]]))
        s.add_op('Z_{i:d}'.format(i=i), np.array([[1.0, 0.0], [0.0, -1.0]]))
        sites.append(s)
    terms = [
        [('X_0', 0)],
        [('X_0', 0), ('X_1', 1)],
    ...
    term_list = TermList(terms, prefactors)
    g1 = mpo.MPOGraph.from_term_list(
        term_list, sites, bc='finite', insert_all_id=False, unit_cell_width=L
    )
    ...
    back_to_terms = H1.to_TermList(
        [['Id', f'X_{i:d}', f'Y_{i:d}', f'Z_{i:d}'] for i in range(L)], max_range=8
    )
...
def test_MPO_plus_identity(sites, alpha=0.7, beta=0.42):
    ...
    H2_expect = mpo.MPOGraph.from_terms(
        (ot_expect, ct_expect), [s] * 4, 'finite', unit_cell_width=4
    ).build_MPO()
...
def test_MPO_expectation_value(tol=1.0e-15):
    ...
    psi1 = mps.MPS.from_singlets(
        s, 6, [(1, 3), (2, 5)], lonely=[0, 4], bc='infinite', unit_cell_width=6
    )
    psi1.test_sanity()
    ...
    H = mpo.MPOGraph.from_terms(
        (ot, ct), [s] * L_mpo, 'infinite', unit_cell_width=L_mpo
    ).build_MPO()
    ...
    L_mpo = 1
    exp_dec_H = mpo.MPO.from_grids(
        [s] * L_mpo, [grid] * L_mpo, bc='infinite', IdL=0, IdR=2, mps_unit_cell_width=L_mpo
    )
    desired_ev = (
        3 * 0.5 * 2  # Sz onsite
        + 0.25
...
def test_MPO_var(L=8, tol=1.0e-13):
    ...
    exp_val_full = npc.inner(
        psi_full, npc.tensordot(ED.full_H, psi_full, axes=1), axes='range', do_conj=True
    )
...
def test_MPOTransferMatrix(eps=1.0e-13):
    ...
    H = mpo.MPO.from_grids(
        [s] * 3, [grid] * 3, 'infinite', 0, 4, max_range=np.inf, mps_unit_cell_width=3
    )
    ...
    # get non-trivial psi
    psi.perturb(
        {'N_steps': 10, 'trunc_params': {'chi_max': 3, 'svd_min': 1.0e-14}},
        close_1=False,
        canonicalize=False,
    )
