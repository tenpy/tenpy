...
def test_mixed_spinless():
    ...
    # real space
    model_params = dict(
        J=J, V=V, lattice='Square', Lx=Lx, Ly=Ly, bc_x='open', bc_y='cylinder', bc_MPS='finite'
    )
    ...
    for i in range(Lx):
        for j in range(Ly):
            ...
            terms_CdC = M.real_to_mixed_correlations_any(
                ['Cd', 'C'], [(1.0, [0, 0])], [(0, 0), (i, j)]
            )
...
def test_mixed_hubbard():
    ...
    # real space
    model_params = dict(
        t=t, U=U, lattice='Square', Lx=Lx, Ly=Ly, bc_x='open', bc_y='cylinder', bc_MPS='finite'
    )
