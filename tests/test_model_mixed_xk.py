...
def test_mixed_spinless():
    ...
    chimax = 100
    # real space
    model_params = dict(J=J, V=V, lattice='Square', Lx=Lx, Ly=Ly, bc_x='open', bc_y='cylinder', bc_MPS='finite')
    ...
    dmrg_params = {
        ...
        'max_E_err': 1.0e-12,
        'max_S_err': 1.0e-8,
        'trunc_params': {'chi_max': chimax, 'svd_min': 1.0e-10},
    }
    product_state = [
    ...
    for i in range(Lx):
        for j in range(Ly):
            ...
            terms_CdC = M.real_to_mixed_correlations_any(['Cd', 'C'], [(1.0, [0, 0])], [(0, 0), (i, j)])
...
def test_mixed_hubbard():
    ...
    chimax = 100
    # real space
    model_params = dict(t=t, U=U, lattice='Square', Lx=Lx, Ly=Ly, bc_x='open', bc_y='cylinder', bc_MPS='finite')
