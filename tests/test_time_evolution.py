...
@pytest.mark.parametrize(
    'bc_MPS, approximation, compression, use_eig_based_svd',
    [
        ('finite', 'I', 'SVD', None),
...
def test_ExpMPOEvolution(bc_MPS, approximation, compression, use_eig_based_svd, g=1.5):
    ...
    model_pars = dict(L=L, Jx=1.0, Jy=1.0, Jz=1.0, hz=0.2, bc_MPS=bc_MPS, conserve='best')
    state = ['up', 'down'] * (L // 2)  # Neel
    M = SpinChain(model_pars)
    psi = MPS.from_product_state(
        M.lat.mps_sites(), state, bc=bc_MPS, unit_cell_width=M.lat.mps_unit_cell_width
    )
    ...
    options = {
        ...
        'trunc_params': {'chi_max': 30, 'svd_min': 1.0e-8},
    }
...
def test_ExpMPOEvolution_dipolar(bc_MPS, approximation, compression, dt=0.01, num_runs=5, L=6):
    if compression == 'variational':
        pytest.xfail('Bond dimension can not grow. Need mixing?')

    model = DipolarSpinChain(dict(L=L, J3=1.0, J4=0.5, bc_MPS=bc_MPS, conserve='dipole'))
    ...
    options = dict(
        ...
        approximation=approximation,
        compression_method=compression,
        trunc_params=dict(chi_max=30, svd_min=1e-8),
    )
...
def fermion_TFI_H(L, g=1.5, J=1.0):
    ...
    return np.concatenate(
        (np.concatenate((A, B), axis=1), np.concatenate((B.conj().T, -A.T), axis=1)), axis=0
    )


def exact_expectation(L, g, t=1.0, dt=0.01):
    ...
    H0 = fermion_TFI_H(L, g=g, J=0.0)
    vp, U0p = LA.eigh(H0)
    assert np.all(vp.round(10) != 0.0)  # so far no handling of zero eigenvalues
    ...
    for t in np.arange(0, t, dt):
        Ub = (
            U @ np.diag(np.exp(-1j * t * v)) @ U.conj().T @ U0
        )  # the total (unitary) Boguliobov transformation
        X = Ub[L::, L::]
        Y = Ub[L::, :L]
        ...
        S = np.concatenate(
            (np.concatenate((A, Nc), axis=1), np.concatenate((M, -A.T), axis=1)), axis=0
        )
        Delta = LA.inv(S)
        ...
        for i in range(L):
            for j in range(L):
                ninj = (
                    -0.25
                    * (-Delta[j + L, i + 3 * L] + Delta[i + L, j + 3 * L])
                    * (-Delta[j + 2 * L, i] + Delta[i + 2 * L, j])
                    + 0.25
                    * (-Delta[j + L, j] + Delta[j + 2 * L, j + 3 * L])
                    * (-Delta[i + L, i] + Delta[i + 2 * L, i + 3 * L])
                    - 0.25
                    * (-Delta[j + L, i] + Delta[i + 2 * L, j + 3 * L])
                ...
                s[i, j] = sij.real if i != j else 1.0
        szsz.append(s)
...
class RabiOscillations(CouplingMPOModel, NearestNeighborModel):
    ...
    def init_terms(self, model_params):
        t = model_params.get('time', 0.0)
        omega0 = model_params.get('omega0', 1.0)
        omega1 = model_params.get('omega1', 1.0)
    ...
    def exact_solution_P1(self, t):
        ...
        omega = self.options.get('omega', 1.0)
        omega0 = self.options.get('omega0', 1.0)
        omega1 = self.options.get('omega1', 1.0)
...
@pytest.mark.slow
def test_time_dependent_evolution(om=0.2 * np.pi, om0=np.pi, om1=0.5 * np.pi, eps=1.0e-4):
    L = 4
