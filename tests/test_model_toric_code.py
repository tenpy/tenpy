...
def test_ToricCode_general():
    check_general_model(
        ToricCode,
        dict(Lx=2, Ly=3, bc_MPS='infinite', sort_charge=True),
        {
...
def test_ToricCode(Lx=1, Ly=2):
    ...
    psi = MPS.from_product_state(
        M.lat.mps_sites(), [0] * M.lat.N_sites, bc='infinite', unit_cell_width=M.lat.mps_unit_cell_width
    )
    dmrg_params = {
        'mixer': True,
        'trunc_params': {'chi_max': 10, 'svd_min': 1.0e-10},
    ...
    assert abs(E - (-1.0)) < dmrg_params['max_E_err']
    print('chi=', psi.chi)
    if Ly == 2:
        assert tuple(psi.chi[:4]) == (2, 2, 2, 2)
    assert abs(psi.entanglement_entropy(bonds=[0])[0] - np.log(2) * (Ly - 1)) < 1.0e-5
