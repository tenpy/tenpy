...
def speigs(A, k=6, M=None, sigma=None, which='LM', v0=None, ncv=None, maxiter=None, tol=0, return_eigenvectors=True, Minv=None, OPinv=None, OPpart=None):
	...
	if k < d - 1:
		...
		if A.shape != (d,d): raise ValueError, "A.shape not a square matrix: " + str(A.shape)
...
"""
	...
	for i in range(len(power_mesh)):
		brute_Ns = (power_mesh[i] if i == 0 else 2 * power_mesh[i]) + 1			# number of points inclusive
		log_power_step = (log_power_range[1]-log_power_range[0]) / float(brute_Ns-1)
		#print 'iteration', i, ":", np.exp(np.arange(brute_Ns) * log_power_step + log_power_range[0])
...
def alg_decay_fits(x, ys, npts=5, power_range=(0.01,4.), power_mesh=[60,10]):
	...
	abc_flat = np.array([ alg_decay_fit(x, yyy, npts=npts, power_range=power_range, power_mesh=power_mesh) for yyy in ys.reshape(-1, len(x)) ])
	...
	if len(x) > 1:
		...
		if 'extrap_line_start' in plot_fit_args:
			try:
				...
				if plot_fit_args['extrap_line_start'] >= len(x): extrap_xrange[0] = np.max(interp_x)
				if plot_fit_args['extrap_line_start'] < -len(x): extrap_xrange[0] = np.min(interp_x)
